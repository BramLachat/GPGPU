...
namespace Intersection {
	...
	//thread per triangle versie 3 - Dit is de beste versie, zie: D:\Masterproef\GPGPU\RayTriangleGPU_v8\bin\win64\Release\Python_Plot\plot_data.py
	//Dit is ook de versie die wordt gebruikt bij All_Versions
	__global__ void intersect_triangleGPU_v3(float3* origins, float dir[3],
		int3* triangles, float3* vertices, int numberOfOrigins, int numberOfTriangles, int* intersectionsPerOrigin)
	{
		int tid = threadIdx.x + blockIdx.x * blockDim.x;
	...
	{
		int threadidx = threadIdx.x; //Probeer ook eens om hier met een offset van 128 te werken per block ==> int threadidx = threadIdx.x + 128*blockIdx.x (zie versie 2)
	...
	/*//block per triangle versie 2 - niet getest omdat v2 van thread per triangle slechter is dan v3 van thread per triangle en deze v2 van block per triangle doet hetzelfde, maar dan voor block per triangle + er zit ook nog een bugske in
	__global__ void intersect_triangleGPU_BlockPerTriangle_v2(float3* origins, float dir[3],
		int3* triangles, float3* vertices, int numberOfOrigins, int* intersectionsPerOrigin) // , int* intersectionsPerOrigin, float3* outsideVertices
	{
		int threadidx = (threadIdx.x + blockDim.x*blockIdx.x) % numberOfOrigins;
		float vert0[3] = { vertices[triangles[blockIdx.x].x].x, vertices[triangles[blockIdx.x].x].y, vertices[triangles[blockIdx.x].x].z };
		float vert1[3] = { vertices[triangles[blockIdx.x].y].x, vertices[triangles[blockIdx.x].y].y, vertices[triangles[blockIdx.x].y].z };
		float vert2[3] = { vertices[triangles[blockIdx.x].z].x, vertices[triangles[blockIdx.x].z].y, vertices[triangles[blockIdx.x].z].z };
		...
		int teller = 0;
		while (teller < numberOfOrigins) {
			if (threadidx < numberOfOrigins) { // Het bugske heeft hier mee te maken!
				float orig[3] = { origins[threadidx].x, origins[threadidx].y, origins[threadidx].z };
			}
			//float orig[3] = { origins[threadidx].x, origins[threadidx].y, origins[threadidx].z };
			float t, u, v;
			...
			threadidx += 128;
			teller += 128;
			//threadidx = threadidx % numberOfOrigins;
			if (teller+128 < numberOfOrigins) {
				threadidx = threadidx % numberOfOrigins;
			}
		}
	}*/
