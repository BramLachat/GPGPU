...
void rayTriangleIntersect(float dir[3], std::unique_ptr<Mesh>& innerMesh, std::unique_ptr<Mesh>& outerMesh)
{
	...
	Intersection::intersect_triangleGPU<<<numberOfBlocks,192>>>(deviceInsideOrigins, cudaDir, deviceOutsideTriangles, deviceOutsideVertices, numberOfCudaCalculations, numberOfOutsideTriangles, d_intersectionsPerThread, d_resultVertices);
